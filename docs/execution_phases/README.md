...
| D5 | "Reddit: search only, never bulk" as a routine Tier-2 source | Reddit's free tier (100 QPM, non-commercial) still exists, but **self-service app registration is closed** — new OAuth credentials require manual approval, typically 2–4 weeks. | Reddit is **not on the critical path**. Apply for credentials during Phase 01 so the clock starts early; ship with HN Algolia + GitHub + Stack Exchange as the community-mining backbone. Reddit degrades to a coverage gap if unapproved. | [01](phase-01-dependency-validation-spike.md), [04](phase-04-search-domain-retrievers.md) |
...
2. **500 MB database ceiling.** `sources.extracted_text` dominates (~1.2 MB/run → ~400 runs). Mitigations: TTL eviction of non-benchmark source text; benchmark sources pinned non-evictable; and a **quote context window** (±2 KB around each cited span) denormalised onto `claims`, so drill-down survives source eviction. See [Phase 00](phase-00-foundation-contracts-ci.md) and [Phase 03](phase-03-fetch-source-cache.md).
...
- [Reddit Data API pricing and access](https://www.socialcrawl.dev/blog/reddit-data-api-2026)
