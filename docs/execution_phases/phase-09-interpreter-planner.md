...
2. **Venue selection for `mine_community`.** With Reddit optional ([D5](README.md#deviations-from-the-masterplan)), should the planner select venues, or should the handler pick from whatever is available? Proposal: planner expresses *intent* ("developer pain points"), handler maps intent to available venues. Keeps the plan stable whether or not Reddit credentials arrive.
